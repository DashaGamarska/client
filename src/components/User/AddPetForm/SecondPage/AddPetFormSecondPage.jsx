import { getPetCommentUser } from 'components/Authorization/Input/inputOptions';
...
export const AddPetFormSecondPage = ({ register, watch, errors, dict }) => {
			...
			<div className={style.uploadThumb}>
				<label htmlFor="petUpload" className={style.petUploadLabel}>
					<p className={style.petUploadLabel__text}>
						{dict.pets.form.photoAndComment}
					</p>
					...
					/>
					{errors['avatar'] && <ValidationError>{dict.pets.form.avatarRequired}</ValidationError>}
				</label>
			</div>
			<AddPetInput
				register={register}
				textarea
				errors={errors}
				settings={getPetCommentUser(dict)}
			/>
		</>
	);
};
