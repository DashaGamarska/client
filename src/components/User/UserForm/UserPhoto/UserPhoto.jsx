...
export const UserPhoto = ({ avatar, watch, register, onSubmit, dict }) => {
				...
				<div className={style.user__uploadThumb}>
					{newImage && newImage[0] && edited ? (
						<button type="submit" className={style.user__uploadSubmit}>
							<UploadSubmit fill="#7db9e8" width={23} height={23} />
							<p className={style.user__uploadText}>
								{dict.userPage.submitPhoto}
							</p>
						</button>
					) : (
						<div>
							<label htmlFor="avatar" className={style.user__uploadLabel}>
								<UploadIcon fill="#7db9e8" width={18} height={18} />
								<p className={style.user__uploadText}>
									{dict.userPage.editPhoto}
								</p>
							</label>
							<input
								type="file"
